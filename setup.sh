...
printf "\e[36m[$0]: 1A. Downloading all required packages first...\e[0m\n"

if (( ${#pkglist[@]} != 0 )); then
    echo -e "\e[34m[$0]: Downloading packages from dependencies.conf...\e[0m"

    for pkg in "${pkglist[@]}"; do
        if pacman -Si "$pkg" &>/dev/null; then
            # Repository package
            v sudo pacman -Sw --noconfirm "$pkg"
        else
            # AUR package
            v yay -Gw "$pkg"
        fi
...
printf "\e[36m[$0]: 1B. Installing packages now...\e[0m\n"
